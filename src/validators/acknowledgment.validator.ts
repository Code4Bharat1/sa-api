...
export const createAcknowledgmentSchema = z.object({
  ...
  trainersPresentCount: z.number().int({ message: 'Trainers count must be a whole number' }).min(1).optional(),
  traineeNames: z.string().trim().max(1000, 'Trainee names cannot exceed 1000 characters').optional(),
...
export const updateAcknowledgmentDateSchema = z.object({
  trainingDate: z.string().refine((val) => !isNaN(Date.parse(val)), {
    message: 'Invalid training date format',
  }),
});
