...
export const createAcknowledgmentSchema = z.object({
  ...
  trainersPresentCount: z.number().int({ message: 'Trainers count must be a whole number' }).min(1, 'At least 1 trainer must be present').max(100, 'Number of trainers cannot exceed 100'),
